...
const DISCOVERY_LINES: Record<TechId, string> = {
  ...
  pottery: 'עיצבה חימר לח והקשיחה אותו באש. כדים! עכשיו אפשר לאגור הרבה יותר לחורף.',
  spear: 'חידדה ענף וחישלה את חודו באש. חנית — ולראשונה יש בשר גם בלב החורף.',
  agriculture: 'טמנה זרעים בשורה וחיכתה. הם נבטו. חקלאות — השדה מאכיל את השבט מעצמו.',
  schooling: 'התיישבה ללמד את הצעירים בשיטה. בית ספר — הדור הבא כבר לא מתחיל מאפס.',
};
